...
export type CityState={player_id:string;username:string;username_claimed:boolean;player:{cash:number;xp:number;power:number;rank:string;level:number};online_count:number;window_seconds:number;poll_seconds:number;season:CitySeason;permissions:string[];events:{id:string;description:string;cash_delta:number;created_at:string}[];server_time:string};
