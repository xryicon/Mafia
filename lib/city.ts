...
export type CityState={player_id:string;username:string;username_claimed:boolean;player:{cash:number;xp:number;level:number};online_count:number;window_seconds:number;poll_seconds:number;season:CitySeason;permissions:string[];events:{id:string;description:string;cash_delta:number;created_at:string}[];server_time:string};
...
export function viewPath(view:GameView,good?:string){
 const path=view==="market"?"/market":view==="businesses"?"/properties":view==="operations"?"/districts":view==="inventory"?"/market?view=inventory":view==="ledger"?"/dashboard?view=ledger":"/dashboard";
