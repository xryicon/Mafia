...
export type DistrictStreet={id:string;district_id:string;name:string;description:string;sort_order:number;archived_at?:string|null};
export type LeaseTerms={template_id:string;rent:number;term_hours:number;enabled:boolean;version:number};
export type PropertyLease={id:string;plot_id:string;building_id:string;player_id:string;player_name:string;ends_at:string;active:boolean};
export type PropertyStorage={building_id:string;plot_id:string;capacity:number;enabled:boolean;can_access:boolean;can_store:boolean;station:{id:string;space:number;installed_at:string}|null};
export type DistrictState={
 streets?:DistrictStreet[];lease_terms?:LeaseTerms[];leases?:PropertyLease[];property_storage?:PropertyStorage[];property_settings?:{station_cost:number;station_space:number};can_manage_properties?:boolean;
 ...
 buy_orders:{id:string;district_id:string;buyer_id:string;buyer_name:string;good_id:string;quantity:number;remaining_quantity?:number;unit_price:number;escrow:number}[];price_comparisons:{good_id:string;local:number|null;city:number|null}[];
