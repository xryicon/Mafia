...
export type ScavengingRisk={level:number;heat:number;searches:number;skill_reduction_percent:number;detection_multiplier:number;heat_per_search:number;cooling_per_minute:number};
export type ScavengingState={district_risk?:Record<string,ScavengingRisk>;session:StreetSession|null;targets:StreetTarget[];streets:string[];settings:Record<string,number>;patrols?:PolicePatrol[];caught?:boolean;operations?:StreetOperations;recent:{kind:string;opened:boolean;xp:number;created_at:string}[]};
