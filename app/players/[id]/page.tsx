...
export default async function Profile({params}:{params:Promise<{id:string}>}){
 ...
 return <main className="control-layout"><div className="control-heading"><div><p className="eyebrow">BLACKWATER / PLAYER PROFILE</p><h1>{data.handle}</h1></div><Link className="button ghost" href="/seasons">Leaderboards</Link></div>
 ...
 <h2>Hall of Fame</h2><p>{data.hall_of_fame.length} archived award{data.hall_of_fame.length===1?"":"s"}</p></main>;
}
