...
export default async function Profile({params}:{params:Promise<{id:string}>}){
 ...
 return <div className="control-layout"><div className="control-heading"><div><p className="eyebrow">BLACKWATER / PLAYER PROFILE</p><PlayerAvatar className="profile-picture-preview" src={data.avatar_url} name={data.handle} size={96}/><h1>{data.handle}</h1></div><Link className="button ghost" href="/seasons">Leaderboards</Link></div>
 {data.is_self&&<ProfilePicture initial={data.avatar_url} name={data.handle}/>}<h2>{data.current_season}</h2><div className="control-grid">{current.map(r=><article className="control-card" key={r.metric}><p>{r.label}</p><strong className="hall-place">#{r.rank}</strong><p>{Number(r.score).toLocaleString("en-US")}</p></article>)}</div>{!current.length&&<p>No current-season ranking yet.</p>}
