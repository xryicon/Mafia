-- Continuous street positions. Coordinates are in blocks, never client-supplied time or speed.
select pg_advisory_xact_lock(4704020);
alter table game_private.scav_sessions add column route_points jsonb;
update game_private.scav_sessions set route_points=(select jsonb_agg(jsonb_build_array(n::int%5,n::int/5) order by ord) from jsonb_array_elements_text(path) with ordinality p(n,ord));
alter table game_private.scav_sessions alter column route_points set default '[[0,0]]';
alter table game_private.scav_sessions alter column route_points set not null;

create function game_private.scav_route_length(points jsonb) returns double precision language plpgsql immutable set search_path='' as $$
declare total double precision:=0; i integer;
begin
 for i in 1..jsonb_array_length(points)-1 loop
  total:=total+abs((points->i->>0)::double precision-(points->(i-1)->>0)::double precision)+abs((points->i->>1)::double precision-(points->(i-1)->>1)::double precision);
 end loop;
 return total;
end$$;

create function game_private.scav_position(points jsonb,departed timestamptz,arrival timestamptz,stamp timestamptz) returns jsonb language plpgsql immutable set search_path='' as $$
declare distance double precision; segment double precision; a jsonb;b jsonb;i integer;
begin
 if arrival<=departed or stamp>=arrival then return points->(jsonb_array_length(points)-1);end if;
 distance:=game_private.scav_route_length(points)*greatest(0,extract(epoch from stamp-departed)/extract(epoch from arrival-departed));
 for i in 1..jsonb_array_length(points)-1 loop
  a:=points->(i-1);b:=points->i;
  segment:=abs((a->>0)::double precision-(b->>0)::double precision)+abs((a->>1)::double precision-(b->>1)::double precision);
  if segment>0 and distance<=segment then return jsonb_build_array((a->>0)::double precision+((b->>0)::double precision-(a->>0)::double precision)*distance/segment,(a->>1)::double precision+((b->>1)::double precision-(a->>1)::double precision)*distance/segment);end if;
...
begin
 ...
 for p in select value from jsonb_array_elements(starts) loop
  for q in select value from jsonb_array_elements(ends) loop
   ...
   distance:=game_private.scav_route_length(candidate);
   if distance<shortest then shortest:=distance;best:=candidate;end if;
  end loop;
 end loop;
 return best;
end$$;
revoke all on function game_private.scav_route_length(jsonb),game_private.scav_position(jsonb,timestamptz,timestamptz,timestamptz),game_private.scav_route(jsonb,jsonb) from public,anon,authenticated;
 ...
 begin
  ...
  if p_action='cancel' then
   ...
   if p_action='enter' then
    ...
    if target is null or game_private.scav_route_length(jsonb_build_array(session.route_points->(jsonb_array_length(session.route_points)-1),jsonb_build_array((target->>'node')::int%5,(target->>'node')::int/5)))>0.000001 then raise exception 'Walk to this location first.';end if;
