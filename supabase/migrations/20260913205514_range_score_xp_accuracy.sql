-- Score-proportional XP and bullseye precision, saved against immutable shot evidence.
select pg_advisory_xact_lock(4704001);
select pg_advisory_xact_lock(4704020);
select set_config('game.reason','Add score-based range XP and saved bullseye accuracy',true);
create or replace function game_private.range_mode_config(mode text) returns jsonb language plpgsql stable set search_path='' as $$
...
begin
 ...
 'move_speed',(c->>'move_speed')::numeric*speed,'completion_xp',reward,'scoring_version',2,'xp_version',2,
 'maximum_score',(c->>'rounds')::int*(c->>'targets_per_round')::int*(c->>'bullseye_points')::int,
...
create function game_private.range_shot_accuracy(p_seed integer,p_round integer,p_elapsed integer,p_x numeric,p_y numeric,c jsonb)
 returns numeric language sql immutable set search_path='' as $$
 select round(max(greatest(0::numeric,1-sqrt(power((p_x-(t.point->>'x')::numeric)/(t.point->>'rx')::numeric,2)+power((p_y-(t.point->>'y')::numeric)/(t.point->>'ry')::numeric,2))))*100,2)
 from generate_series(0,(c->>'targets_per_round')::int-1) lane
 cross join lateral (select game_private.range_target(p_seed,p_round,lane,p_elapsed,c) as point)t;
$$;
create table game_private.range_shot_precision(
 shot_id uuid primary key references public.game_range_shots(id),
 session_id uuid not null references public.game_range_sessions(id),
 accuracy_percent numeric(5,2) not null check(accuracy_percent between 0 and 100)
);
create index range_precision_session on game_private.range_shot_precision(session_id) include(accuracy_percent);
alter table game_private.range_shot_precision enable row level security;
revoke all on game_private.range_shot_precision from public,anon,authenticated;
create trigger range_precision_retain before update or delete or truncate on game_private.range_shot_precision for each statement execute function game_private.district_immutable();
-- Append derived precision for historical shots without changing their immutable evidence or XP.
insert into game_private.range_shot_precision(shot_id,session_id,accuracy_percent)
 select q.id,q.session_id,game_private.range_shot_accuracy(s.seed,q.round,q.elapsed_ms,q.x,q.y,s.config)
 from public.game_range_shots q join public.game_range_sessions s on s.id=q.session_id;
create function game_private.range_store_precision() returns trigger language plpgsql set search_path='' as $$
begin
 insert into game_private.range_shot_precision(shot_id,session_id,accuracy_percent)
  select new.id,new.session_id,game_private.range_shot_accuracy(s.seed,new.round,new.elapsed_ms,new.x,new.y,s.config) from public.game_range_sessions s where s.id=new.session_id;
 return new;
end$$;
create trigger range_store_precision after insert on public.game_range_shots for each row execute function game_private.range_store_precision();
create function game_private.range_session_accuracy(p_session uuid) returns numeric language sql stable set search_path='' as $$
 select round(avg(accuracy_percent),2) from game_private.range_shot_precision where session_id=p_session;
$$;
revoke all on function game_private.range_score_xp(integer,jsonb),game_private.range_shot_accuracy(integer,integer,integer,numeric,numeric,jsonb),game_private.range_store_precision(),game_private.range_session_accuracy(uuid) from public,anon,authenticated;
create or replace function game_private.range_complete_session() returns trigger language plpgsql security definer set search_path='' as $$
...
begin
 ...
 and (select count(distinct round) from public.game_range_shots where session_id=old.id)>=(old.config->>'required_rounds')::int then
  award:=game_private.range_score_xp(old.score,old.config);
  if coalesce(award,0)<=0 then return new;end if;
...
begin
 ...
 'session',case when v.id is null then null else to_jsonb(v)||jsonb_build_object('accuracy_percent',game_private.range_session_accuracy(v.id),'score_xp',game_private.range_score_xp(v.score,v.config),'participated_rounds',(select count(distinct round) from public.game_range_shots where session_id=v.id),'hit_targets',(select coalesce(jsonb_agg(round::text||':'||lane),'[]') from public.game_range_shots where session_id=v.id and hit),'last_shot',(select jsonb_build_object('id',shot_row.id,'hit',shot_row.hit,'points',shot_row.points,'round',shot_row.round,'lane',shot_row.lane,'x',shot_row.x,'y',shot_row.y,'elapsed_ms',shot_row.elapsed_ms,'created_at',shot_row.created_at,'accuracy_percent',(select accuracy_percent from game_private.range_shot_precision where shot_id=shot_row.id)) from public.game_range_shots shot_row where shot_row.session_id=v.id order by shot_row.created_at desc,shot_row.id desc limit 1)) end,
 'stats',(select jsonb_build_object('xp_earned',coalesce(sum(xp_awarded),0),'sessions',count(*),'best_score',coalesce(max(score),0),'shots',coalesce(sum(shots),0),'hits',coalesce(sum(hits),0),'best_accuracy',coalesce(max(game_private.range_session_accuracy(id)),0)) from public.game_range_sessions where season_id=s and player_id=u and status<>'active'),
 'advanced_stats',(select jsonb_build_object('sessions',count(*),'shots',coalesce(sum(r.shots),0),'average_accuracy',round(sum(a.total)/nullif(sum(r.shots),0),2),'best_accuracy',max(a.average),'xp_earned',coalesce(sum(r.xp_awarded),0))
 from public.game_range_sessions r cross join lateral (select sum(accuracy_percent) as total,round(avg(accuracy_percent),2) as average from game_private.range_shot_precision where session_id=r.id)a
 where r.season_id=s and r.player_id=u and r.difficulty='advanced' and r.status<>'active' and r.shots>0),
 'recent',(select coalesce(jsonb_agg(x order by x.started_at desc),'[]') from (select id,score,hits,shots,status,started_at,difficulty,xp_awarded,game_private.range_session_accuracy(id) as accuracy_percent from public.game_range_sessions where season_id=s and player_id=u and status<>'active' order by started_at desc limit 5) x),
 ...
 begin
  ...
  if p_action='start' then
   ...
   if p_action='finish' then
    ...
    result:=jsonb_build_object('message',case when v.xp_awarded>0 then 'Session complete. +'||v.xp_awarded||' XP.' when v.status='stopped' then 'Session ended early. No completion XP.' else 'Session recorded. XP depends on your score and completing the required rounds.' end,'session_id',v.id,'xp_awarded',v.xp_awarded);
