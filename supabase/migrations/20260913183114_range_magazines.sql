-- Persistent magazines prepare equipped ammunition; only shots consume bullets.
select pg_advisory_xact_lock(4704020);
select set_config('game.reason','Add persistent shooting-range magazines and timed reloads',true);
alter table public.game_range_weapons add column magazine_capacity integer not null default 10 check(magazine_capacity between 1 and 1000);
alter table public.game_range_weapons add column reload_ms integer not null default 1800 check(reload_ms between 500 and 15000);
create table game_private.range_magazines(
 weapon_id uuid primary key references public.game_inventory_gear(id),season_id uuid not null references public.game_seasons(id),player_id uuid not null references public.game_players(id),
 ammo_id uuid not null references public.game_inventory_gear(id),loaded integer not null check(loaded between 0 and 1000),
 reload_capacity integer check(reload_capacity between 1 and 1000),reload_started_at timestamptz,reload_ready_at timestamptz,fire_ready_at timestamptz,
 check((reload_started_at is null)=(reload_ready_at is null)),check(reload_ready_at is null or reload_capacity is not null)
);
create index range_magazines_owner on game_private.range_magazines(player_id,season_id);
create index range_magazines_season on game_private.range_magazines(season_id);
create index range_magazines_ammo on game_private.range_magazines(ammo_id);
alter table game_private.range_magazines enable row level security;
revoke all on game_private.range_magazines from public,anon,authenticated;
create trigger range_magazines_retain before delete or truncate on game_private.range_magazines for each statement execute function game_private.district_immutable();
-- Existing active sessions adopt magazine rules without changing bullets or weapon wear.
update public.game_range_sessions s set weapon_rule=s.weapon_rule||jsonb_build_object('magazine_capacity',w.magazine_capacity,'reload_ms',w.reload_ms)
 from public.game_range_weapons w where s.weapon_good_id=w.good_id and s.status='active';
insert into game_private.range_magazines(weapon_id,season_id,player_id,ammo_id,loaded)
 select distinct on(s.weapon_id) s.weapon_id,s.season_id,s.player_id,a.id,least(a.quantity,greatest(0,w.magazine_capacity-s.shots))
 from public.game_range_sessions s join public.game_range_weapons w on w.good_id=s.weapon_good_id
 join public.game_inventory_gear g on g.id=s.weapon_id and g.location='equipped'
 join public.game_inventory_gear a on a.season_id=s.season_id and a.player_id=s.player_id and a.location='equipped' and a.equipment_slot='ammo' and a.good_id=w.ammo_good_id and a.quantity>0
 where s.status='active' order by s.weapon_id,s.started_at desc;

create function game_private.range_magazine_state(gid uuid,rules jsonb) returns jsonb language plpgsql set search_path='' as $$
declare m game_private.range_magazines;a public.game_inventory_gear;capacity int:=(rules->>'magazine_capacity')::int;n int:=0;t timestamptz:=clock_timestamp();
begin
 select * into m from game_private.range_magazines where weapon_id=gid for update;
 select * into a from public.game_inventory_gear where player_id=auth.uid() and season_id=game_private.current_season() and location='equipped' and equipment_slot='ammo' and good_id=rules->>'ammo_good_id' and quantity>0;
 if m.weapon_id is null then n:=least(coalesce(capacity,0),coalesce(a.quantity,0));
 elsif m.ammo_id=a.id then
  if m.reload_ready_at is not null and m.reload_ready_at<=t then
   update game_private.range_magazines set loaded=least(reload_capacity,capacity,a.quantity),fire_ready_at=reload_ready_at,reload_ready_at=null,reload_started_at=null where weapon_id=gid returning * into m;
  end if;
  n:=least(m.loaded,coalesce(a.quantity,0),coalesce(capacity,0));
 end if;
 return jsonb_build_object('loaded',n,'capacity',capacity,'reserve',greatest(0,coalesce(a.quantity,0)-n),'reloading',coalesce(m.ammo_id=a.id and m.reload_ready_at>t,false),'started_at',m.reload_started_at,'ready_at',m.reload_ready_at,'reload_ms',(rules->>'reload_ms')::int);
end$$;
-- Removing a weapon or its ammunition unloads it and cancels pending reloads.
create function game_private.range_unload_equipment() returns trigger language plpgsql security definer set search_path='' as $$
begin
 if old.location='equipped' and (new.location,new.equipment_slot) is distinct from (old.location,old.equipment_slot) then
  update game_private.range_magazines set loaded=0,reload_started_at=null,reload_ready_at=null where weapon_id=old.id or ammo_id=old.id;
 end if;
 return new;
end$$;
create trigger range_unload_equipment after update on public.game_inventory_gear for each row execute function game_private.range_unload_equipment();
revoke all on function game_private.range_magazine_state(uuid,jsonb),game_private.range_unload_equipment() from public,anon,authenticated;
