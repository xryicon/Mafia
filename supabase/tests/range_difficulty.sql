...
begin
 ...
 r:=public.inventory_action('equip',jsonb_build_object('season_id',s,'request_id',gen_random_uuid(),'item_key','good:homemade-pistol','equipment_slot','secondary'));perform pg_temp.range_check(not r?'error','Equip pistol: '||r::text);
 r:=public.inventory_action('equip',jsonb_build_object('season_id',s,'request_id',gen_random_uuid(),'item_key','good:homemade-bullets','equipment_slot','ammo','quantity',30));perform pg_temp.range_check(not r?'error','Equip ammo: '||r::text);
 ...
 q:=q||jsonb_build_object('request_id',gen_random_uuid(),'difficulty','beginner');r:=public.range_action('start',q);perform pg_temp.range_check(not r?'error','Beginner start: '||r::text);
 ...
 for i in 0..2 loop
  ...
  perform pg_temp.range_check(not r?'error','Scoring shot: '||r::text);points:=array_append(points,(r->>'points')::int);perform pg_sleep(.22);
 end loop;
 ...
 elapsed:=floor(extract(epoch from(clock_timestamp()-v.started_at))*1000)::int;
 r:=public.range_action('fire',jsonb_build_object('season_id',s,'request_id',gen_random_uuid(),'session_id',v.id,'elapsed_ms',elapsed,'x',50,'y',45));perform pg_temp.range_check(not r?'error','Second-round shot: '||r::text);
 ...
 r:=public.range_action('start',jsonb_build_object('season_id',s,'request_id',gen_random_uuid(),'equipment_slot','secondary','difficulty','advanced'));perform pg_temp.range_check(not r?'error','Advanced start: '||r::text);
 select * into v from public.game_range_sessions where player_id=u and status='active';
 elapsed:=floor(extract(epoch from(clock_timestamp()-v.started_at))*1000)::int;
 r:=public.range_action('fire',jsonb_build_object('season_id',s,'request_id',gen_random_uuid(),'session_id',v.id,'elapsed_ms',elapsed,'x',50,'y',45));perform pg_temp.range_check(not r?'error','Advanced fire: '||r::text);
 ...
 update public.game_range_sessions set cooldown_until=clock_timestamp()-interval '1 second' where player_id=u;
 r:=public.range_action('start',jsonb_build_object('season_id',s,'request_id',gen_random_uuid(),'equipment_slot','secondary'));perform pg_temp.range_check(not r?'error','Idle start: '||r::text);
 ...
 update public.game_range_sessions set cooldown_until=clock_timestamp()-interval '1 second' where player_id=u;
 r:=public.range_action('start',jsonb_build_object('season_id',s,'request_id',gen_random_uuid(),'equipment_slot','secondary','difficulty','beginner'));perform pg_temp.range_check(not r?'error','Season boundary start: '||r::text);
