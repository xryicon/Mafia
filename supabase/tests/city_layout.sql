...
begin
 ...
 if (result->'player'->>'power')::bigint is distinct from 10 then raise exception 'Power does not match server respect'; end if;
 reset role;
 update public.game_settings set value=case key when 'rank_soldier' then 10 when 'rank_caporegime' then 100 when 'rank_underboss' then 1000 end where key in ('rank_soldier','rank_caporegime','rank_underboss');
 set local role authenticated;
 result:=public.city_status();
 if result->'player'->>'rank' is distinct from 'Soldier' then raise exception 'Header ignores configured Soldier threshold'; end if;
 reset role;
 update public.game_settings set value=10 where key='rank_caporegime';
 set local role authenticated;result:=public.city_status();
 if result->'player'->>'rank' is distinct from 'Caporegime' then raise exception 'Header ignores configured Caporegime threshold'; end if;
 reset role;
 update public.game_settings set value=10 where key='rank_underboss';
 set local role authenticated;result:=public.city_status();
 if result->'player'->>'rank' is distinct from 'Underboss' then raise exception 'Header ignores configured Underboss threshold'; end if;
