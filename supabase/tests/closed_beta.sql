...
do $$
declare owner_id uuid:=gen_random_uuid();player_id uuid:=gen_random_uuid();v jsonb;blocked boolean:=false;launch integer:=1906567200;
begin
 ...
 insert into auth.users(id) values(owner_id),(player_id);
 perform set_config('request.jwt.claim.sub',owner_id::text,true);perform public.game_state();
 perform set_config('request.jwt.claim.sub',player_id::text,true);perform public.game_state();
