...
export function binWorld(game,mining){
 ...
 for(const [id,name] of [["pickaxe","Pickaxe"],["pistol_blueprint","Homemade pistol blueprint"],["bullet_blueprint","Homemade bullet blueprint"]])if(!game.goods.some(g=>g.id===id))game.goods.push({id,name,business_name:"Found loot",business_cost:1,batch_size:1,cycle_seconds:60,business_available:false});
 ...
 function read(){return {season:game.season,server_time:new Date().toISOString(),playable:game.season.status==="open",can_manage:admin,rules,districts:catalog,cash:game.player.cash,inventory:Object.fromEntries(game.inventory.filter(i=>["pickaxe","pistol_blueprint","bullet_blueprint"].includes(i.good_id)).map(i=>[i.good_id,i.quantity])),tool_condition:mining.read().tool.durability,tool_max:100,mining_shift:!!mining.read().shift,ready_at:ready,history};}
