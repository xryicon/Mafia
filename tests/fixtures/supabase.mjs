import {chopWorld} from "./chop-shop.mjs";
...
import {robberyWorld} from "./robbery.mjs";
...
let scavenging=scavWorld(state,()=>bins,()=>skills,()=>{prisonSentence={id:"patrol-sentence",reason:"Caught by a police patrol while scavenging",started_at:new Date().toISOString(),release_at:new Date(Date.now()+300000).toISOString()};});
...
const server = http.createServer(async(req,res) => {
 ...
 if(url.pathname.startsWith("/rest/v1/rpc/chop_shop_")){let raw="";for await(const chunk of req)raw+=chunk;const p=JSON.parse(raw||"{}");send(200,url.pathname.endsWith("chop_shop_state")?chop.read():chop.action(p.p_action,p.p_payload));return;}
 ...
 if(url.pathname==="/rest/v1/rpc/telegram_preferences"){let raw="";for await(const chunk of req)raw+=chunk;const p=JSON.parse(raw||"{}");if(p.p_preferences)alertPreferences=p.p_preferences;send(200,alertPreferences);return;}
 if(url.pathname==="/rest/v1/rpc/market_price_history"){let raw="";for await(const chunk of req)raw+=chunk;const p=JSON.parse(raw||"{}");const rows=p.p_good==="whiskey"?Array.from({length:12},(_,i)=>({source_kind:i%2?"order":"auction",source_id:String(i),quantity:2,total:200,unit_price:100,created_at:new Date(Date.now()-i*86400000).toISOString()})):[];send(200,{good_id:p.p_good,total:rows.length,units:rows.length*2,average:rows.length?100:null,low:rows.length?100:null,high:rows.length?100:null,recent:rows.slice(p.p_offset??0,(p.p_offset??0)+10),daily:rows.map(r=>({day:r.created_at.slice(0,10),units:2,average:100})).reverse()});return;}
 ...
 if(url.pathname==="/__buy_orders_setup"){
  let raw="";for await(const chunk of req)raw+=chunk;const p=JSON.parse(raw||"{}");
  if(p.listings)state.market=Array.from({length:p.listings},(_,i)=>({...state.market[0],id:"listing-"+i,unit_price:100+i}));
  districts.buy_orders=Array.from({length:p.count??12},(_,i)=>({id:"order-"+i,district_id:districts.district.id,buyer_id:"33333333-3333-4333-8333-333333333333",buyer_name:"Buyer "+i,good_id:"whiskey",quantity:5,remaining_quantity:5,unit_price:100,escrow:500}));send(200,{});return;
 }
 if(url.pathname==="/rest/v1/rpc/district_market_order"){
  ...
  if(buyOrderRequests.has(p.request_id)){send(200,buyOrderRequests.get(p.request_id));return;}
  const order=districts.buy_orders.find(o=>o.id===p.order_id);
  if(action==="create"){const total=Number(p.quantity)*Number(p.unit_price);if(total>state.player.cash){send(200,{error:"Not enough cash"});return;}state.player.cash-=total;districts.buy_orders.push({id:crypto.randomUUID(),district_id:p.district_id,buyer_id:playerId,buyer_name:state.player.handle,good_id:p.good_id,quantity:Number(p.quantity),remaining_quantity:Number(p.quantity),unit_price:Number(p.unit_price),escrow:total});}
  if(action==="cancel"){if(!order||order.buyer_id!==playerId){send(200,{error:"Not your order"});return;}state.player.cash+=order.escrow;districts.buy_orders=districts.buy_orders.filter(o=>o.id!==order.id);}
  if(action==="fill"){const qty=Number(p.quantity),stock=state.inventory.find(i=>i.good_id===order?.good_id);if(!order||order.buyer_id===playerId||qty<1||qty>order.remaining_quantity||!stock||stock.quantity<qty){send(200,{error:"Invalid quantity"});return;}stock.quantity-=qty;order.remaining_quantity-=qty;order.escrow-=qty*order.unit_price;state.player.cash+=qty*order.unit_price-Math.ceil(qty*order.unit_price*state.settings.market_fee_percent/100);if(!order.remaining_quantity)districts.buy_orders=districts.buy_orders.filter(o=>o.id!==order.id);}
  const result={message:action==="create"?"Buy order funded and posted.":action==="fill"?"Goods supplied. Payment received.":"Buy order cancelled. Funds returned."};buyOrderRequests.set(p.request_id,result);send(200,result);return;
 }
