...
const server = http.createServer(async(req,res) => {
 ...
 if(url.pathname.startsWith('/rest/v1/rpc/bin_diving_')){let raw='';for await(const chunk of req)raw+=chunk;const p=JSON.parse(raw||'{}');send(200,url.pathname.endsWith('bin_diving_state')?{...bins.read(),scavenging:scavenging.read(),robbery:robberies.read()}:bins.action(p.p_action,p.p_payload));return;}
