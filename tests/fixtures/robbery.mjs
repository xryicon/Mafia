...
export function robberyWorld(game,street){
 ...
 const read=()=>({season_id:game.season.id,server_time:new Date().toISOString(),district_id:street().session?.district_id??null,rules,factors:{power:game.player.xp,level:4,accuracy:62.5,attack:20,defense:10},weapon,available:!!street().session&&!street().session?.pending,ready_at,protected_until:null,targets,history,management:game.permissions.includes("roles.manage")?{gear}:null});
 const action=(kind,p)=>{
  if(kind==="configure"||kind==="gear"){if(!game.permissions.includes("roles.manage"))return {error:"Owner robbery permission required."};if(kind==="configure"){if(p.version!==rules.version)return {error:"Rules changed."};Object.assign(rules,p.rules);rules.version++;}else{const g=gear.find(g=>g.id===p.good_id);Object.assign(g,{attack:p.attack,defense:p.defense,condition_max:p.condition_max,version:g.version+1});rules.version++;}return {message:"Robbery settings saved and audited."};}
  ...
  const result={message:"Robbery succeeded. Stole $250. Used 3 bullets.",cash:250,bullets:3,succeeded:true};requests.set(p.request_id,{key,result});return result;
 };
