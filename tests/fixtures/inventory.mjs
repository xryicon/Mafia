export function inventoryWorld(state,bin,setTool=()=>{}){
 ...
 let version=1,nextGear=1,health=100;
 ...
 const action=(kind,p)=>{
  ...
  try{
   ...
   if(kind==="scrap_weapon"){if(!g||g.condition!==0||g.quantity!==1||!["carried","equipped"].includes(g.location)||goods().find(i=>i.id===g.good_id)?.category!=="weapons")fail("Only your broken weapons can be scrapped.");g.quantity=0;g.location="retired";g.equipment_slot=null;if(!fits("scrap-metal",1))fail("Inventory full.");let row=state.inventory.find(i=>i.good_id==="scrap-metal");if(!row){row={good_id:"scrap-metal",quantity:0};state.inventory.push(row);}row.quantity++;entry("scrap-metal",1,"carried",null,row.quantity);}
   else if(kind==="use_bandage"){if(health>=100)fail("Your health is already full. No bandage was used.");if(g){if(g.good_id!=="bandages"||!["carried","equipped"].includes(g.location))fail("Select your carried bandage.");g.quantity--;if(!g.quantity){g.location="retired";g.equipment_slot=null;}}else{const row=state.inventory.find(x=>x.good_id==="bandages");if(p.item_key!=="good:bandages"||!row?.quantity)fail("Carry a bandage first.");row.quantity--;}health=Math.min(100,health+20);}
   else if(kind==="move"){if(p.version!==version)fail("Inventory changed. Review the refreshed slots and try again.");reorder(p.item_key,p.position);}
   else if(kind==="equip"){
 ...
 const setup=p=>{if(p.brokenWeapon){for(const good of [{id:"homemade-pistol",name:"Homemade pistol",inventory_meta:{category:"weapons",weight_grams:1500,equipment_slots:["secondary"]}},{id:"scrap-metal",name:"Scrap metal",inventory_meta:{category:"materials",weight_grams:250,equipment_slots:[]}}])if(!state.goods.some(g=>g.id===good.id))state.goods.push(good);gear.push({id:"broken-test",good_id:"homemade-pistol",quantity:1,condition:0,location:"equipped",equipment_slot:"secondary",building_id:null});}if(p.bandages){health=65;state.goods.push({id:"bandages",name:"Bandages",business_available:false,inventory_meta:{category:"medical",weight_grams:100,equipment_slots:["medical"]}});state.inventory.push({good_id:"bandages",quantity:3});}if(p.full){state.inventory=state.goods.map((g,i)=>({good_id:g.id,quantity:g.id==="pickaxe"?3:g.id.endsWith("_blueprint")?2:3+i%3}));}
 ...
 const delivery=(good_id,quantity,reason)=>deliveries.push({id:"craft-delivery-"+(deliveries.length+1),good_id,quantity,reason,created_at:new Date().toISOString()}); const rangeShot=(weaponId,ammoId,wear,maximum)=>{const gun=gear.find(g=>g.id===weaponId),ammo=gear.find(g=>g.id===ammoId);gun.condition=Math.max(0,Math.min(maximum,gun.condition??maximum)-wear);ammo.quantity--;entry(ammo.good_id,-1,"equipped",null,ammo.quantity);if(!ammo.quantity){ammo.location="retired";ammo.equipment_slot=null;}};return {read,action,manage,setup,cityLease,station,delivery,rangeShot};
}
