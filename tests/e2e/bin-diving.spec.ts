...
async function enterStreet(page:Page){await expect(page.locator(".simple-scav")).toBeVisible();const enter=page.getByRole("button",{name:/^Enter /});if(await enter.count())await enter.click();await expect(page.locator(".simple-scav-grid")).toBeVisible();}
async function searchBin(page:Page){await enterStreet(page);await page.getByRole("button",{name:"Search bin",exact:true}).first().click();await expect(page.locator(".simple-scav-status")).toBeVisible();await expect.poll(async()=>await page.locator(".simple-scav-status").count()===0||await page.getByRole("button",{name:"Retry safely"}).count()>0,{timeout:15000}).toBe(true);}
...
test("Owner grants reach the stash and both market sale formats, with matching artwork",async({page})=>{
 ...
 await page.goto("/bin-diving");await expect(page.locator(".bin-stash-row>b")).toHaveText(["0","0","1","1","0","0","0"]);await page.locator("summary").filter({hasText:"Equipment & recent finds"}).click();await expect(page.getByRole("button",{name:"Equip pickaxe",exact:false})).toBeDisabled();
