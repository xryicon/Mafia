...
async function searchBin(page:Page){await enterStreet(page);await page.getByRole("button",{name:"Search bin",exact:true}).first().click();await expect(page.locator(".simple-scav-status")).toBeVisible();await expect(page.locator(".simple-scav-status")).toHaveCount(0,{timeout:15000});}
