...
test("public mining buys a pickaxe, completes one shift, and makes gathered resources tradeable",async({page,request})=>{
 ...
 await capture(page,"mining-overview-desktop");
 await page.getByRole("button",{name:"View MQ-01 North Ridge Iron Mine",exact:true}).click();
 const detail=page.locator(".mine-desktop-details");await detail.getByRole("button",{name:"Buy pickaxe · $100",exact:true}).click();await expect(page.locator(".header-cash")).toContainText("$9,900");
 await detail.getByRole("button",{name:"Start mining shift",exact:true}).click();
 await expect(page.getByRole("button",{name:"Collect mined resources",exact:true})).toBeDisabled();await expect(page.locator(".mine-supply")).toContainText("99 condition");
 ...
 await page.getByRole("button",{name:"My mining",exact:true}).click();await expect(page.locator(".mine-inventory")).toContainText("4 units");await expect(page.locator(".mine-history")).toContainText("4 Iron ore");
 await page.locator(".mine-inventory").getByRole("link").first().click();await expect(page).toHaveURL(/\/market\?good=iron-ore&view=inventory/);await expect(page.locator(".market-inventory")).toContainText("Iron ore");
...
test("interrupted mining requests retry safely without another pickaxe charge",async({page})=>{
 await page.goto("/districts/mines-and-quarries");let interrupted=false;await page.route("**/rest/v1/rpc/mining_action",async route=>{if(!interrupted){interrupted=true;await route.fetch();await route.abort("failed");}else await route.continue();});
 await page.locator(".mine-supply").getByRole("button",{name:/Buy pickaxe/}).click();await page.getByRole("button",{name:"Retry same request"}).click();await expect(page.locator(".mine-notice")).toContainText("Pickaxe purchased");await expect(page.locator(".header-cash")).toContainText("$9,900");await expect(page.locator(".mine-supply")).toContainText("100 condition");
});
