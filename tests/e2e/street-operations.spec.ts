...
async function steal(page:Page){await page.goto("/bin-diving");await page.getByRole("button",{name:/^Enter /}).click();const garage=page.getByLabel("Store stolen vehicles in");if(await garage.count())await garage.selectOption({index:1});await page.getByRole("button",{name:/^Steal vehicle/}).first().click();await page.getByRole("button",{name:"Start getaway",exact:true}).click();await expect(page.locator(".simple-scav-status")).toContainText("Police pursuit");}
async function escape(page:Page){await page.getByRole("button",{name:"Run to the exit"}).click();await page.getByRole("button",{name:"Escape district",exact:true}).click();}
test("stolen car automatically sells without a garage; pursuit survives reload and map fits mobile",async({page,request})=>{
 ...
 await page.reload();await expect(page.locator(".simple-scav-status")).toContainText("Police pursuit");
...
test("garage owners keep stolen vehicles and can sell exactly once after an interrupted response",async({page,request})=>{
 ...
 await page.locator("summary").filter({hasText:"Vehicles, police risk & street record"}).click();page.once("dialog",d=>d.accept());await page.getByRole("button",{name:"Sell vehicle",exact:true}).click();await page.getByRole("button",{name:"Retry safely"}).click();await expect(page.locator(".street-vehicle")).toHaveCount(0);await expect(page.locator(".bin-notice")).toContainText("Vehicle sold");
});
...
test("street events are actionable and the new page uses readable Blackwater artwork",async({page,request})=>{
 ...
 await page.getByRole("button",{name:/^Enter /}).click();await page.getByRole("button",{name:"Investigate opportunity",exact:true}).click();await expect(page.locator(".bin-result")).toContainText("Pickaxe");await expect(page.locator(".simple-scav-event")).toHaveCount(0);
});
