...
async function steal(page:Page){await page.goto("/bin-diving?view=aerial");await page.getByRole("button",{name:/^Enter /}).click();await page.locator('.scav-target[aria-label^="Car"]').first().click();await page.getByRole("button",{name:/^Steal vehicle/}).click();await page.getByRole("button",{name:"Start getaway",exact:true}).click();await expect(page.locator(".scav-workspace")).toHaveClass(/pursued/);}
...
test("street events are actionable and the new page uses readable Blackwater artwork",async({page,request})=>{
 await request.post(fixture+"/__street_setup",{headers,data:{street:true,event:"satchel"}});await page.setViewportSize({width:1600,height:1000});await page.goto("/bin-diving?view=aerial");await expect(page.locator(".street-hero")).toHaveCSS("background-image",/street-operations.webp/);await page.screenshot({path:"test-results/street-entry-desktop.png"});
 await page.getByRole("button",{name:/^Enter /}).click();await page.getByRole("button",{name:"Dropped courier satchel",exact:true}).click();await page.getByRole("button",{name:"Investigate opportunity",exact:true}).click();await page.waitForResponse(r=>r.url().includes("/rpc/scavenging_action")&&r.request().postDataJSON().p_action==="finish");await expect(page.locator(".bin-result")).toContainText("Pickaxe");await expect(page.locator(".street-event-marker")).toHaveCount(0);
});
