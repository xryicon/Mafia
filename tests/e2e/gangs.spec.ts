...
test("statements paginate and the gang layout works on mobile and desktop",async({page,request})=>{
 ...
 await page.setViewportSize({width:390,height:844});await page.getByRole("navigation",{name:"Gang sections"}).getByRole("button",{name:"members",exact:true}).click();await capture(page,"gangs-mobile-members");await page.getByRole("button",{name:"Change rank"}).first().click();expect(await page.getByRole("dialog").evaluate(e=>e.getBoundingClientRect().width)).toBeLessThan(390);
});
