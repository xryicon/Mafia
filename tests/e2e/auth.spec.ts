...
test("private pages redirect anonymous visitors to login", async ({ page }) => {
  for (const path of ["/skills","/inventory","/bank", "/dashboard", "/dashboard/private", "/update-password", "/players", "/owner", "/support", "/account", "/seasons", "/market", "/properties", "/districts", "/gangs", "/profile", "/telegrams", "/ledger"]) {
