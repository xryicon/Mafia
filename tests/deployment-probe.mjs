...
for(const path of [...(styles.includes(".skills-workspace")?["/skills"]:[]),"/dashboard","/market","/properties","/gangs","/seasons","/support","/players","/districts","/districts/the-waterfront","/districts/mines-and-quarries","/districts/manage","/telegrams"]){
...
if(commodityArtworkDeployed){
 ...
 if(styles.includes(".m4-arsenal-art"))for(const name of ["m4-carbine","556x45mm-ammo"]){
  for(const suffix of ["","-256","-96"]){
...
if(rangeRecordings.ok){
 const recordings=["indoor-range","colt-1911-shot","1911-reload"];
 if(styles.includes(".range-automatic-fire"))recordings.push("m4-carbine-shot","m4-carbine-reload");
