...
export function GameShell({ initial }: { initial: GameState }) {
  ...
  return <div className="game-shell">
    <aside className="game-sidebar"><Brand className="game-wordmark"/><div className="sidebar-rule" /><p className="sidebar-label">YOUR EMPIRE</p><nav className="game-nav" aria-label="Game navigation">{tabs.map(item => <button key={item.id} className={tab === item.id ? "active" : ""} aria-current={tab === item.id ? "page" : undefined} onClick={() => openTab(item.id)}><GameIcon name={item.id}/><span>{item.label}</span>{item.id === "market" && <span className="nav-count">{state.market.length}</span>}</button>)}</nav><div className="sidebar-bottom"><div className="season-card"><GameIcon name="shield" size={25}/><span>{state.season.name}<br/><small>{state.season.status} · Season standings ↗</small></span></div><Link className="account-link" href="/seasons">Seasons & leaderboards ↗</Link><Link className="account-link" href={"/players/"+state.player.id}>Your profile ↗</Link><Link className="account-link" href="/support">Support ↗</Link>{state.permissions.length>0&&<Link className="account-link" href={state.permissions.includes("roles.manage")?"/owner":"/staff"}>{state.permissions.includes("roles.manage")?"Owner panel":"Staff panel"} ↗</Link>}<Link className="account-link" href="/account">Your account <span>↗</span></Link><LogoutButton /></div><div className="sidebar-version"><span className="green-dot" /> PERSISTENT WORLD <span>V0.2</span></div></aside>
    <div className="game-main">
      ...
      <div className="game-content">
        ...
        {tab === "overview" && <>
          <section className="district-hero"><Artwork name="exchange" priority/><div className="office-shade"/><div className="district-top"><span className="tag">DISTRICT 01</span><span>THE DOCKS / BLACKWATER</span></div><div className="district-copy"><p className="eyebrow">YOUR OFFICE. YOUR NEXT MOVE.</p><h2>A city of deals.<br/><em>A name to remember.</em></h2><p>Build the supply. Read the market.<br/>Turn an opportunity into your next deal.</p><div className="hero-buttons"><button className="button" onClick={() => openTab("operations")}>Find an operation <GameIcon name="arrow" size={17}/></button><button className="button ghost" onClick={() => openTab("market")}>Visit the market</button></div></div><div className="district-stamp">BW<span>THE BLACKWATER EXCHANGE</span></div></section>
          <MarketPulse state={state} onChoose={id=>{setFilter(id);setTab("market");}}/><div className="overview-columns"><section className="game-panel"><div className="panel-heading"><div><p className="eyebrow">PLAYER-DRIVEN ECONOMY</p><h2>Latest market offers</h2></div><button className="link-button" onClick={() => openTab("market")}>View market <GameIcon name="arrow" size={15}/></button></div>{marketRows(state.market.slice(0,4))}<div className="panel-foot"><GameIcon name="shield" size={14}/> Goods held until sale. {settings.market_fee_percent}% seller fee. No artificial buyers.</div></section>
        ...
        <footer className="game-footer"><span>BLACKWATER MAFIA · THE CITY RUNS ON DEALS.</span><span>Game currency only · Market refreshes every 30 seconds</span></footer>
      </div>
    </div>
