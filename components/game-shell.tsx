...
export function GameShell({ initial }: { initial: GameState }) {
  ...
  function businessCards() {
    return <div className="business-grid">{state.goods.map(good => {
      ...
      return <article className={"business-card " + good.id} key={good.id}><div className="business-art"><span>{symbols[good.id]}</span><span className={"tag " + (owned ? "green-tag" : "")}>{owned ? "OWNED" : "FOR SALE"}</span><div className="business-silhouette" /></div><div className="business-body"><p className="eyebrow">{good.id === "whiskey" ? "THE DOCKS" : good.id === "silk" ? "OLD TOWN" : "INDUSTRIAL QUARTER"}</p><h3>{good.business_name}</h3><p>{businessCopy[good.id]}</p><div className="production-line"><GameIcon name="inventory" size={15}/><span>{good.batch_size} {good.name.toLowerCase()} / {good.cycle_seconds} sec</span></div>{owned ? <><div className="business-price"><span>{units ? "READY TO COLLECT" : "NEXT BATCH"}</span><strong className={units ? "green" : ""}>{units ? units + " units" : duration(next)}</strong></div><button className="button full small" disabled={busy || units === 0} onClick={() => act("collect", { good_id: good.id })}>{units ? "Collect production" : "Production in progress"}</button></> : <><div className="business-price"><span>ACQUISITION COST</span><strong className="gold">{money(good.business_cost)}</strong></div><button className="button full small ghost" disabled={busy || state.player.cash < good.business_cost} onClick={() => act("business", { good_id: good.id })}>Buy business · {money(good.business_cost)}</button></>}</div></article>;
    })}</div>;
  }

  return <div className="game-shell">
    <aside className="game-sidebar"><Link href="/" className="wordmark game-wordmark">MAFIA<span>BLACKWATER</span></Link><div className="sidebar-rule" /><p className="sidebar-label">YOUR EMPIRE</p><nav className="game-nav" aria-label="Game navigation">{tabs.map(item => <button key={item.id} className={tab === item.id ? "active" : ""} aria-current={tab === item.id ? "page" : undefined} onClick={() => openTab(item.id)}><GameIcon name={item.id}/><span>{item.label}</span>{item.id === "market" && <span className="nav-count">{state.market.length}</span>}</button>)}</nav><div className="sidebar-bottom"><div className="season-card"><GameIcon name="shield" size={25}/><span>BLACKWATER<br/><small>Make a name. Build an empire.</small></span></div><Link className="account-link" href="/seasons">Seasons & leaderboards ↗</Link><Link className="account-link" href={"/players/"+state.player.id}>Your profile ↗</Link><Link className="account-link" href="/community">Community & support ↗</Link>{state.permissions.length>0&&<Link className="account-link" href="/staff">{state.permissions.includes("roles.manage")?"Owner panel":"Staff panel"} ↗</Link>}<Link className="account-link" href="/update-password">Account security <span>↗</span></Link><LogoutButton /></div><div className="sidebar-version"><span className="green-dot" /> PERSISTENT WORLD <span>V0.2</span></div></aside>
    <div className="game-main">
      <header className="game-topbar"><div className="breadcrumb">BLACKWATER <span>/</span> {tabs.find(t => t.id === tab)?.label.toUpperCase()}</div><div className="topbar-right"><button className="refresh-button" onClick={refresh} disabled={pending || refreshing} aria-label="Refresh city data"><GameIcon name="refresh" size={17}/><span>{refreshing ? "Refreshing…" : "Refresh"}</span></button><div className="player-identity"><span className="avatar">{state.player.handle.slice(0,1)}</span><div><strong>{state.player.handle}</strong><small>{rank(state.player.xp, settings)}</small></div></div></div></header>
      <div className="game-content">
        <div className="season-status"><Link href="/seasons">{state.season.name}</Link> · {state.season.status}{!playing && " · Gameplay is paused. Your accounts and season history are preserved."}</div><div className="page-heading"><div><p className="eyebrow">EVERY MOVE LEAVES A MARK</p><h1>{tab === "overview" ? "Your empire." : tab === "market" ? "The black market." : tab === "operations" ? "Work the streets." : tab === "businesses" ? "Own the supply." : tab === "inventory" ? "Your inventory." : "Follow the money."}</h1><p>{tab === "overview" ? "The city is moving. Make sure it's moving in your favor." : tab === "market" ? "Real players. Real offers. You decide what your goods are worth." : tab === "operations" ? "Earn cash and respect. All operations share one crew cooldown." : tab === "businesses" ? "Build a supply chain. Collect completed batches and trade with the city." : tab === "inventory" ? "Turn the goods in your warehouse into your next deal." : "Every operation, acquisition, and trade in one place."}</p></div><span className="world-badge"><span className="red-dot" /> BLACKWATER / EST. 2026</span></div>
        ...
        {tab === "businesses" && <><div className="info-strip"><GameIcon name="businesses"/><span>Each business has its own production cycle. Collect up to {settings.offline_batches} batches at once, then sell them to other players.</span></div>{businessCards()}</>}
        {tab === "market" && <><section className="game-panel"><div className="panel-heading"><div><p className="eyebrow">LATEST 200 OPEN OFFERS</p><h2>Trading floor</h2></div><button className="button small" onClick={() => openTab("inventory")}>Create an offer <GameIcon name="arrow" size={15}/></button></div><div className="market-filters" aria-label="Filter commodities">{[{id:"all",name:"All goods"},...state.goods].map(g => <button key={g.id} className={filter === g.id ? "selected" : ""} aria-pressed={filter === g.id} onClick={() => setFilter(g.id)}>{g.name}</button>)}</div>{marketRows(market)}<div className="panel-foot">Whole lots only · {settings.market_fee_percent}% seller fee, rounded up to the next dollar · Updates every 30 seconds</div></section><section className="game-panel own-offers"><div className="panel-heading"><h2>Your active offers</h2><span className="muted">{state.my_listings.length} / {settings.listing_limit}</span></div>{state.my_listings.length ? marketRows(state.my_listings) : <p className="panel-empty">You haven't listed any goods. Start from your inventory.</p>}</section></>}
