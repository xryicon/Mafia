...
export function GameShell({ initial, initialTab="overview", initialGood, initialDistrict }: { initial: GameState; initialTab?:Tab; initialGood?:string; initialDistrict?:string }) {
  ...
  function marketRows(offers: Listing[]) {
    return offers.length ? <div className="table-scroll"><table className="market-table"><thead><tr><th>Commodity / seller</th><th>Quantity</th><th>Per unit</th><th>Lot total</th><th><span className="sr-only">Trade</span></th></tr></thead><tbody>{offers.map(offer =>
      <tr key={offer.id}><td><div className="commodity"><CommodityArtwork goodId={offer.good_id} size={44}/><div><strong>{goodsName(offer.good_id)}</strong><small>{offer.seller_id === state.player.id ? "YOUR OFFER" : offer.seller_handle}</small></div></div></td><td>{offer.quantity}</td><td className="gold">{money(offer.unit_price)}</td><td>{money(offer.quantity * offer.unit_price)}</td><td>{offer.seller_id === state.player.id ? <button className="table-button muted-button" disabled={busy} onClick={() => act("cancel", { listing_id: offer.id })}>Withdraw</button> : <button className="table-button" disabled={busy || state.player.cash < offer.quantity * offer.unit_price} onClick={() => setPurchase(offer)}>{state.player.cash < offer.quantity * offer.unit_price ? "Low cash" : "Buy lot"}</button>}</td></tr>
  ...
  return <div className={"estate-game view-"+tab}>
   {tab==="businesses"?<PropertyEstate state={state} now={now} propertyId={propertyId} busy={busy} onAction={act} onSelect={id=>{setPropertyId(id);router.push(viewPath("businesses",id));}} onSell={id=>openTab("inventory",id)} notice={<>{notice && <div className={"game-notice " + (failed ? "error" : "")} role={failed ? "alert" : "status"}>{powerText(notice)}<button onClick={() => setNotice("")} aria-label="Dismiss message">×</button></div>}</>}/>:<>
    ...
    <div className="stats-grid estate-stats"><article><span className="stat-icon gold"><GameIcon name="cash"/></span><div><span>CASH ON HAND</span><strong className="gold">{money(state.player.cash)}</strong><small>Available to invest & trade</small></div></article><article><span className="stat-icon violet"><GameIcon name="respect"/></span><div><span>POWER</span><strong>{state.player.xp.toLocaleString("en-US")} <i>Power</i></strong><small>{rank(state.player.xp, settings)}</small></div></article><article><span className="stat-icon blue"><GameIcon name="inventory"/></span><div><span>WAREHOUSE STOCK</span><strong>{totalStock.toLocaleString("en-US")} <i>units</i></strong><small>{state.my_listings.reduce((sum,l) => sum + l.quantity, 0)} reserved on the market</small></div></article><article><span className="stat-icon green"><GameIcon name="businesses"/></span><div><span>BUSINESSES</span><strong>{state.businesses.length} <i>/ {state.goods.length}</i></strong><small>{state.businesses.length ? "Your production network" : "Your first acquisition awaits"}</small></div></article></div>
    {!playing&&<p className="game-notice">This season is paused. Visit Seasons for the current standings.</p>}
    <>{notice && <div className={"game-notice " + (failed ? "error" : "")} role={failed ? "alert" : "status"}>{powerText(notice)}<button onClick={() => setNotice("")} aria-label="Dismiss message">×</button></div>}</>
            {tab === "overview" && <>
          ...
          <section className="game-panel next-move"><p className="eyebrow">YOUR NEXT MOVE</p><h2>Keep the cash flowing.</h2><p>Every operation earns cash and power. Your crew needs a little time between jobs.</p><div className="operation-mini"><GameIcon name="operations" size={27}/><div><strong>Dock errand</strong><small>THE DOCKS · {jobs.find(j=>j.id==='docks')?.cooldown} SEC COOLDOWN</small></div></div><div className="reward-row"><strong className="gold">+{money(jobs.find(j=>j.id==='docks')?.reward || 0)}</strong><span className="violet">+{jobs.find(j=>j.id==='docks')?.xp} power</span></div><button className="button full" disabled={busy || cooldown > 0} onClick={() => act("job", { job: "docks" })}>{cooldown > 0 ? "Crew ready in " + duration(cooldown) : "Complete dock errand"}</button><button className="link-button centered" onClick={() => openTab("operations")}>All operations <GameIcon name="arrow" size={15}/></button></section></div>
