...
export function TelegramWorkspace({initial}:{initial:TelegramState}){
 ...
 async function roomAct(action:string,payload:Record<string,unknown>={}){
  ...
  try{
   ...
   if(["leave","decline"].includes(task.action)){setThread(null);setMembers(false);}
   await refresh();window.dispatchEvent(new Event("blackwater:game"));return r.data;
 ...
 <aside className="tg-panel tg-sidebar"><h2 className="tg-sidebar-title">YOUR CORRESPONDENCE</h2><button className="tg-gold tg-new" disabled={locked} onClick={()=>newTelegram()}><GameIcon name="mail"/>New Telegram</button><button className="tg-new-group" disabled={locked} onClick={()=>setNewGroup(true)}><GameIcon name="people"/>New Group</button>
