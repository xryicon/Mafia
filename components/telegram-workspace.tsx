...
import {TelegramPreferences} from "./telegram-preferences";
...
export function TelegramWorkspace({initial,initialRecipient=""}:{initial:TelegramState;initialRecipient?:string}){
 const [data,setData]=useState(initial),[folder,setFolder]=useState("inbox"),[thread,setThread]=useState<string|null>(null),[before,setBefore]=useState<string|null>(null),[offset,setOffset]=useState(0),[compose,setCompose]=useState(!!initialRecipient),[showOffice,setShowOffice]=useState(false);
 const [recipient,setRecipient]=useState(initialRecipient),[subject,setSubject]=useState(""),[body,setBody]=useState(""),[draftId,setDraftId]=useState<string|null>(null),[candidates,setCandidates]=useState<{id:string;username:string}[]>([]);
