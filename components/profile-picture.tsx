...
export function ProfilePicture({initial,name}:{initial?:string|null;name:string}){
 ...
 async function save(e:FormEvent<HTMLFormElement>){e.preventDefault();setBusy(true);setNotice("");try{const r=await createClient().rpc("profile_avatar",{p_url:url.trim()});if(r.error||r.data?.error)throw new Error(r.data?.error||r.error?.message);setSaved(r.data.avatar_url);setNotice(r.data.message);window.dispatchEvent(new Event("blackwater:identity"));}catch(e){setNotice(e instanceof Error?e.message:"Picture could not be saved.");}finally{setBusy(false);}}
