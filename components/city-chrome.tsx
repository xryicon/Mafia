...
export function CityChrome({children}:{children:React.ReactNode}){
 ...
 const blank=["/dashboard","/properties","/districts","/telegrams"].includes(path);
 const active=(href:string)=>path===href||path.startsWith(href+"/");
 return <div className={"estate-city fresh-city"+(blank?" fresh-page":"")}>
  <header className="estate-header"><Brand href="/dashboard" className="estate-brand"/><nav className="estate-nav" aria-label="Game navigation">{links.map(([href,name,icon])=><Link href={href} key={href} aria-current={active(href)?"page":undefined}><GameIcon name={icon} size={18}/><span>{name}</span></Link>)}</nav>
  <div className="estate-player-tools"><div className="header-cash header-wallet" aria-label="Cash balance">{data?money(data.player.cash):"—"}</div>
  <details className="estate-account-menu" ref={account}><summary aria-label="Player menu"><Portrait/><div><span className="estate-player-name">{data?.username||"Your account"} <i>⌄</i></span><span className="estate-player-numbers"><span>{data&&!stale?data.online_count.toLocaleString("en-US"):"—"} online</span><span>LV {data?.player.level??"—"}</span></span></div></summary>
   <div className="estate-dropdown"><p className="eyebrow">YOUR BLACKWATER</p><p className="account-identity">{data?.username||"Your account"}<small>{data&&!stale?data.online_count:"—"} online · Level {data?.player.level??"—"}</small></p><Link href="/players">Players & respect</Link><Link href="/seasons?view=rankings">Leaderboards</Link><Link href="/seasons">Seasons</Link><Link href="/support">Support</Link>{!!data?.permissions.length&&<Link className="owner-menu-link" href={data.permissions.includes("roles.manage")?"/owner":"/staff"}>{data.permissions.includes("roles.manage")?"Owner panel":"Staff panel"}</Link>}<Link href="/ledger">Financial history</Link><Link href="/account">Account & security</Link><LogoutButton/></div>
  </details></div></header>
