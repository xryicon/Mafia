...
export function CityChrome({children}:{children:React.ReactNode}){
 ...
 const command=path==="/dashboard";
 const [summary,setSummary]=useState<{stock:number;unread:number|null}|null>(null);
 ...
 return <div className={"estate-city fresh-city"+(blank?" fresh-page":"")+(command?" command-city":"")}>
  <header className="estate-header"><Brand href="/dashboard" className="estate-brand" status={<><span className={"presence-dot"+(stale?" offline":"")}/><span>{data&&!stale?data.online_count.toLocaleString("en-US"):"—"} online</span></>}/>{command&&<span className="command-header-tagline">A PLAYER-DRIVEN CRIME ECONOMY</span>}<nav className="estate-nav" aria-label="Game navigation">{links.map(([href,name,icon])=><Link href={href} key={href} aria-current={active(href)?"page":undefined}><GameIcon name={icon} size={18}/><span>{name}</span>{command&&href==="/telegrams"&&!!summary?.unread&&<b className="command-nav-badge" aria-hidden="true">{summary.unread}</b>}</Link>)}</nav>
  ...
  {command&&<Link href="/market?view=inventory" className="command-header-stock" aria-label="Inventory stock"><GameIcon name="inventory" size={22}/>{summary?.stock.toLocaleString("en-US")??"—"}</Link>}
  {command&&<Link className="command-header-bell" href="/telegrams" aria-label="Telegram notifications"><GameIcon name="bell" size={21}/>{!!summary?.unread&&<b aria-hidden="true">{summary.unread}</b>}</Link>}
  <details className="estate-account-menu" ref={account}><summary aria-label="Player menu">{command?<img className="don-portrait" src="/art/command-portrait.jpg" alt="" width={36} height={36}/>:<Portrait/>}<div><span className="estate-player-name">{data?.username||"Your account"} <i>⌄</i></span><span className="estate-player-numbers"><span className="header-power">Power <strong>{data?.player.power?.toLocaleString("en-US")??"—"}</strong></span><span className="header-rank" title="Rank">{data?.player.rank||"—"}</span></span></div></summary>
