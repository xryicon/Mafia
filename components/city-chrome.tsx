...
export function CityChrome({children}:{children:React.ReactNode}){
 ...
 return <div className={"estate-city fresh-city command-city"+(blank?" fresh-page":"")+(path==="/dashboard"?" command-dashboard-page":"")}>
  ...
  <details className="estate-account-menu" ref={account}><summary aria-label="Player menu"><PlayerAvatar className="don-portrait" src={data?.avatar_url} size={36}/><div><span className="estate-player-name">{data?.username||"Your account"} <i>⌄</i></span><span className="estate-player-numbers"><span className="header-power">Power <strong>{data?.player.power?.toLocaleString("en-US")??"—"}</strong></span><span className="header-rank" title="Rank">{data?.player.rank||"—"}</span></span></div></summary>
   <div className="estate-dropdown"><p className="eyebrow">YOUR BLACKWATER</p><p className="account-identity">{data?.username||"Your account"}<small>Power {data?.player.power?.toLocaleString("en-US")??"—"} · {data?.player.rank||"—"}</small></p><Link href="/players">Players & power</Link><Link href="/seasons?view=rankings">Leaderboards</Link><Link href="/seasons">Seasons</Link><Link href="/support">Support</Link>{!!data?.permissions.length&&<Link className="owner-menu-link" href={data.permissions.includes("roles.manage")?"/owner":"/staff"}>{data.permissions.includes("roles.manage")?"Owner panel":"Staff panel"}</Link>}<Link href="/districts">City map & districts</Link><Link href="/skills">Skills</Link><Link href="/shooting-range">Shooting range</Link><Link href="/ledger">Financial history</Link><Link href="/account">Account & security</Link><LogoutButton/></div>
  </details></div></header>
