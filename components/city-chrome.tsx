...
export function CityChrome({children}:{children:React.ReactNode}){
 ...
 const alive=useRef(false),reading=useRef(false),account=useRef<HTMLDetailsElement>(null),activity=useRef<HTMLDetailsElement>(null);
 ...
 const links=[["/dashboard","Dashboard"],["/market","Market"],["/properties","Properties"],["/districts","Districts"],["/gangs","Gangs"],["/profile","Profile"]];
 const active=(href:string)=>path===href||path.startsWith(href+"/")||(href==="/profile"&&path.startsWith("/players/"));
 return <div className="estate-city">
  <header className="estate-header"><Brand href="/dashboard" className="estate-brand"/><nav className="estate-nav" aria-label="Game navigation">{links.map(([href,name])=><Link href={href} key={href} aria-current={active(href)?"page":undefined}>{name}</Link>)}</nav>
  <div className="estate-player-tools"><details className="estate-activity-menu" ref={activity}><summary aria-label="Recent activity"><GameIcon name="bell" size={22}/></summary><div className="estate-dropdown"><p className="eyebrow">RECENT ACTIVITY</p>{data?.events.length?data.events.map(e=><p className="header-event" key={e.id}>{e.description}<small>{new Date(e.created_at).toLocaleString("en-GB",{timeZone:"UTC"})} UTC</small></p>):<p>Your next move will appear here.</p>}<Link href="/dashboard?view=ledger">View your ledger →</Link></div></details>
  <details className="estate-account-menu" ref={account}><summary aria-label="Player menu"><Portrait/><div><span className="estate-player-name">{data?.username||"Your account"} <i>⌄</i></span><span className="estate-player-numbers"><span className="header-cash"><GameIcon name="cash" size={15}/>{data?money(data.player.cash):"—"}</span><span><GameIcon name="people" size={15}/>{data&&!stale?data.online_count.toLocaleString("en-US"):"—"}<small className="sr-only"> online</small></span><span><GameIcon name="respect" size={15}/>LV {data?.player.level??"—"}</span></span></div></summary>
   <div className="estate-dropdown"><p className="eyebrow">YOUR BLACKWATER</p><Link href="/profile">Your profile</Link><Link href="/players">Players & respect</Link><Link href="/seasons?view=rankings">Leaderboards</Link><Link href="/seasons">Seasons</Link><Link href="/support">Support</Link>{!!data?.permissions.length&&<Link className="owner-menu-link" href={data.permissions.includes("roles.manage")?"/owner":"/staff"}>{data.permissions.includes("roles.manage")?"Owner panel":"Staff panel"}</Link>}<Link href="/account">Account & security</Link><LogoutButton/></div>
  </details></div></header>
  ...
  <footer className="estate-footer"><Brand href="/dashboard"/><p>A PLAYER-DRIVEN CRIME ECONOMY <span>│</span> AMBITION LIVES HERE.</p><nav aria-label="Game information"><Link href="/seasons">Seasons</Link><Link href="/players">Players</Link><Link href="/support">Help</Link></nav><small>SAME PLAYERS.<br/>A DIFFERENT TOMORROW.</small></footer>
