...
export function DistrictPropertyHeader({state}:{state:DistrictState}){
 ...
 <div className="dp-banner-foot"><span>{state.season.name}</span><span>Trade builds empires.</span></div>
...
export function DistrictProperties({state,onSelect,onSection}:{state:DistrictState;onSelect:(id:string)=>void;onSection:(tab:"Plots"|"Businesses"|"Territory"|"Activity")=>void}){
 ...
 const streets=state.streets??[],active=streets.find(s=>s.id===street)??streets[0],others=state.plots.some(p=>!streets.some(s=>s.id===p.street_id));
 const current=street==="all"?null:active,visible=state.plots.filter(p=>{
  ...
  return (!current||p.street_id===current.id)&&(p.code+" "+name+" "+p.owner_name).toLowerCase().includes(query.toLowerCase())
   &&(filter==="all"||filter==="rent"&&!!lease&&p.owner_type==="city"||filter==="sale"&&(p.status==="available"||p.asking_price!==null)||filter==="mine"&&(p.owner_id===state.player_id||state.leases?.some(l=>l.plot_id===p.id&&l.player_id===state.player_id&&l.active))||filter==="storage"&&state.property_storage?.some(s=>s.plot_id===p.id))&&!(filter==="rent"&&held);
 });
 ...
 <nav className="dp-streets" aria-label="District streets">{streets.map(s=><button key={s.id} aria-current={current?.id===s.id?"page":undefined} onClick={()=>setStreet(s.id)}><GameIcon name="districts" size={22}/><span>{s.name}<small>{state.plots.filter(p=>p.street_id===s.id).length} properties</small></span></button>)}
 ...
 <div className="dp-body"><section className="dp-properties"><header className="dp-heading"><div><h2>{current?.name||"District properties"}</h2><p>{current?.description||"Find your place in the Blackwater economy."}</p></div><span>{visible.length} properties</span></header>
 <div className="dp-filters"><label><span>Find a property</span><input placeholder="Property, plot or owner…" value={query} onChange={e=>setQuery(e.target.value)}/></label><label><span>Availability</span><select value={filter} onChange={e=>setFilter(e.target.value)}><option value="all">All properties</option><option value="rent">City leases available</option><option value="sale">For sale</option><option value="mine">Your properties & leases</option><option value="storage">Garages & warehouses</option></select></label></div>
 <div className="dp-cards">{visible.map(p=>{
  ...
  const rent=!!terms&&p.owner_type==="city",owned=p.owner_id===state.player_id,mine=lease?.player_id===state.player_id;
  const status=owned?"Your property":mine?"Your lease":lease?"Leased":rent?"For lease":p.asking_price!==null||p.status==="available"?"For sale":p.status==="owned"?"Owned":p.status;
  ...
  <button className="dp-outline" onClick={()=>onSelect(p.id)}>{owned||mine?"Manage property":rent&&!lease?"View lease":"View property"}<GameIcon name="arrow" size={17}/></button></div></article>;
 })}</div>{!visible.length&&<div className="dp-empty"><GameIcon name="property" size={36}/><h3>No matching properties</h3><p>Try another street or change your filters.</p><button className="dp-outline" onClick={()=>{setStreet("all");setFilter("all");setQuery("");}}>Show all district properties</button></div>}
...
export function PropertyFacilities({p,state,act,busy}:{p:Plot;state:DistrictState;act:DistrictAction;busy:boolean}){
 ...
 {store.can_store&&settings&&<div className="dp-station"><GameIcon name="tools" size={34}/><div><h3>Crafting station</h3>{store.station?<><p>Installed inside this property. {store.station.space} storage units reserved.</p><p>No crafting recipes are available yet.</p><button className="dp-outline" disabled={busy} onClick={()=>confirm==="remove_station"?void run("remove_station"):setConfirm("remove_station")}>{confirm==="remove_station"?"Confirm removal · no refund":"Remove station"}</button></>:<><p>Fit a permanent workbench inside your garage or warehouse.</p><p>{money(settings.station_cost)} installation · {settings.station_space} storage units required.</p><button className="dp-outline" disabled={busy||state.cash<settings.station_cost||!store.enabled} onClick={()=>confirm==="install_station"?void run("install_station",{cost:settings.station_cost,space:settings.station_space}):setConfirm("install_station")}>{confirm==="install_station"?"Confirm installation":"Install crafting station"}</button></>}</div></div>}
 </section>;
}
