...
export function MarketPulse({state,onChoose}:{state:GameState;onChoose:(id:string)=>void}){
 ...
 return <button className={"pulse-card "+g.id} key={g.id} onClick={()=>onChoose(g.id)}><span className="pulse-commodity"><GameIcon name="market" size={16}/>{g.name}<GameIcon name="arrow" size={15}/></span><strong>{lowest===null?"Make the first offer":money(lowest)}{lowest!==null&&<small> lowest ask / unit</small>}</strong><span className="pulse-foot">{units.toLocaleString()} units on offer <i>·</i> {offers.length} listing{offers.length===1?"":"s"}</span></button>;
 })}</div></section>;
}
