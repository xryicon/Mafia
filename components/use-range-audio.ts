...
export function useRangeAudio(){
 ...
 const unlock=useCallback((weaponGoodId?:string)=>{if(!engine.current)engine.current=new RangeAudio(()=>{if(alive.current)setError("A range recording could not load. Toggle sound to retry.");});const p=prefs.current;void engine.current.unlock(p.enabled,p.volume/100,weaponGoodId).then(()=>{if(alive.current)setError("");}).catch(()=>{if(alive.current)setError("Sound is unavailable. You can still use the range.");});},[]);
 ...
 return {enabled,volume,error,unlock,toggle:()=>change({...prefs.current,enabled:!prefs.current.enabled}),setVolume:(volume:number)=>change({...prefs.current,volume}),shot:(weaponGoodId?:string)=>play(()=>engine.current?.shot(weaponGoodId)),reload:(ms:number,elapsed:number,weaponGoodId?:string)=>play(()=>engine.current?.reload(ms,elapsed,weaponGoodId))};
}
