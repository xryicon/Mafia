...
import {ScavengingMap} from "./scavenging-map";
...
export function BinDiving({initial,initialDistrict}:{initial:BinState;initialDistrict?:string}){
 ...
 <div className="bin-breadcrumb"><Link href="/dashboard">← Dashboard</Link><span>/</span><Link href="/districts">City map</Link><span>/ SCAVENGING</span>{data.can_manage&&<Link className="bin-owner-link" href="/owner?section=bin-diving">Owner controls ↗</Link>}</div>
 <OperationHeader title="Scavenging" kicker="THE STREETS" motto="One man’s trash. Your next opportunity." icon="bin" status={playable&&data.rules.enabled?"The streets are open":"Searching is paused"} caption={data.season.name}/>
 ...
 <ScavengingMap robbery={data.robbery} state={data.scavenging} district={district} now={h.now} busy={h.busy} blocked={!playable||!data.rules.enabled||closed} cooldown={seconds} lockpicks={data.inventory.lockpick??0} act={h.act}/>
 </div>
 ...
 <aside className="bin-panel bin-stash"><header><h2>Your street stash</h2><GameIcon name="inventory" size={19}/></header><p className="bin-muted">Finds from this season. Ready when you need them.</p>{loot.filter(l=>l.id!=="cash").map(l=><div className="bin-stash-row" key={l.id}><LootArt item={l.id}/><span><strong>{l.name}</strong><small>{l.id==="pickaxe"?"Spare equipment":l.id==="lockpick"?"Single-use lockpick":"Trade or save for crafting"}</small></span><b>{data.inventory[l.id as LootItem]??0}</b><Link className="bin-trade-link" aria-label={"Trade "+l.name} href={"/market?view=inventory&good="+l.id}>Trade ↗</Link></div>)}
 ...
 <div className="bin-stash-note"><GameIcon name="shield" size={19}/><p>Trade your finds at the player market or keep them for later. Items listed for sale or auction are reserved until sold or withdrawn. Equipped pickaxes cannot be traded.</p></div></aside>
 </div>
 <section className="bin-panel bin-history"><header><h2>Your recent searches</h2><span className="bin-muted">LATEST 30 / THIS SEASON</span></header>{data.history.length?<ol>{data.history.map(r=><li key={r.id}><LootArt item={r.outcome}/><div><strong>{r.outcome==="cash"?money(r.cash)+" found":loot.find(l=>l.id===r.outcome)?.name??"Nothing found"}</strong><span>{r.district_name}</span></div><time dateTime={r.created_at}>{new Date(r.created_at).toLocaleString()}</time></li>)}</ol>:<p className="bin-muted">Your first search will leave a story here.</p>}</section>
 </div>;
}
