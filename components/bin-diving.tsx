...
export function BinDiving({initial,initialDistrict}:{initial:BinState;initialDistrict?:string}){
 ...
 <aside className="bin-panel bin-stash"><header><h2>Your street stash</h2><GameIcon name="inventory" size={19}/></header><p className="bin-muted">Finds from this season. Ready when you need them.</p>{loot.filter(l=>l.id!=="cash").map(l=><div className="bin-stash-row" key={l.id}><LootArt item={l.id}/><span><strong>{l.name}</strong><small>{l.id==="pickaxe"?"Spare equipment":"Trade or save for crafting"}</small></span><b>{data.inventory[l.id as LootItem]??0}</b><Link className="bin-trade-link" aria-label={"Trade "+l.name} href={"/market?view=inventory&good="+l.id}>Trade ↗</Link></div>)}
