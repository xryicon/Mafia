...
export function LootArt({item}:{item:string}){
 if(["pickaxe","lockpick","pistol_blueprint","bullet_blueprint","bandages_blueprint","reinforced_jacket_blueprint","kevlar_vest_blueprint"].includes(item))return <span className={"bin-loot-art illustrated "+item}><CommodityArtwork goodId={item} size={96}/></span>;
...
export function BinDiving({initial,initialDistrict}:{initial:BinState;initialDistrict?:string}){
 ...
 <header className="street-hero"><div><p className="eyebrow">BLACKWATER / AFTER DARK</p><h1 aria-label="Scavenging">Scavenging</h1><p>Choose a district, search a bin or lockpick a car, and collect your finds. Watch the police risk.</p><div className="street-hero-tags"><span>SCAVENGE</span><span>STEAL</span><span>ESCAPE</span></div></div><span className="street-hero-motto">FORTUNE FAVOURS<br/>THE WATCHFUL.</span></header>
 <div className="street-district-bar"><label>Operating district<select aria-label="District" disabled={h.busy||!!data.scavenging?.operations?.pursuit||!!data.scavenging?.session?.pending||!!data.scavenging?.session&&Date.parse(data.scavenging.session.arrives_at)>h.now} value={district?.id??""} onChange={e=>setSelection(e.target.value)}><option value="" disabled>Select a district</option>{data.districts.map(d=><option key={d.id} value={d.id}>{d.name}{d.slug==="blackwater-island"?" · High risk":d.status==="lockdown"?" · Lockdown":""}</option>)}</select></label><div className="street-wallet"><span>ON HAND</span><strong>{money(data.cash)}</strong></div><button className="bin-outline" onClick={h.refresh} aria-label="Refresh bin diving"><GameIcon name="refresh" size={18}/>Refresh</button></div>
 ...
 {result&&<section className={"bin-panel bin-result "+(result&&result.outcome!=="nothing"?"found":"")} role="status" aria-live="polite"><LootArt item={result?.outcome??"nothing"}/><div><p className="eyebrow">LATEST FIND</p><h2>{result?result.outcome==="cash"?money(result.cash)+" in loose cash":found?.name??"Nothing this time.":"Every street has a story."}</h2><p>{result?result.district_name+" · "+(result.outcome==="cash"?"Added to your wallet.":result.outcome==="nothing"?"No loot this time. Check Skills for your current Scavenging progress.":"Added to Inventory. Store valuable finds in your garage or warehouse."):"Search bins and cars, investigate marked events or attempt a vehicle theft."}</p><div className="simple-scav-result-links"><Link href="/inventory">Open inventory →</Link><Link href="/skills">View skills →</Link></div></div></section>}
 <DistrictTravelGate target={district?.id??""}><ScavengingChoices state={data.scavenging} district={district} now={h.now} busy={h.busy} blocked={!playable||!data.rules.enabled||closed} cooldown={seconds} lockpicks={data.inventory.lockpick??0} act={h.act}/></DistrictTravelGate>
 <details className="street-loot-guide"><summary>Vehicles, police risk & street record</summary><StreetOperationsPanel ops={data.scavenging?.operations} busy={h.busy} act={h.act}/></details>

 <details className="street-loot-guide"><summary>Loot guide & current chances <span>Owner-configured</span></summary><section className="bin-panel bin-odds"><div className="bin-loot-grid">{loot.map(l=><article key={l.id}><LootArt item={l.id}/><strong>{l.name}</strong><span className="bin-chance">{data.rules[(l.id+"_chance") as keyof typeof data.rules]}%</span><p>{l.id==="cash"?money(data.rules.cash_min)+"–"+money(data.rules.cash_max)+" base reward":l.description}</p></article>)}</div><p className="bin-muted">{emptyChance(data.rules)}% chance of finding nothing. Base item chances stay the same across districts. Blackwater Island and street events multiply cash finds. <Link href="/inventory">Manage your inventory →</Link></p></section></details>
<details className="street-loot-guide"><summary>Equipment & recent finds</summary><div className="street-support"> <aside className="bin-panel bin-stash"><header><h2>Your street stash</h2><GameIcon name="inventory" size={19}/></header><p className="bin-muted">Finds from this season. Ready when you need them.</p>{loot.filter(l=>l.id!=="cash").map(l=><div className="bin-stash-row" key={l.id}><LootArt item={l.id}/><span><strong>{l.name}</strong><small>{l.id==="pickaxe"?"Spare equipment":l.id==="lockpick"?"Single-use lockpick":"Trade or save for crafting"}</small></span><b>{data.inventory[l.id as LootItem]??0}</b><Link className="bin-trade-link" aria-label={"Trade "+l.name} href={"/market?view=inventory&good="+l.id}>Trade ↗</Link></div>)}
