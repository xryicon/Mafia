...
export function useRange(initial:RangeState){
 const [data,setData]=useState(initial),[working,setWorking]=useState(false),[notice,setNotice]=useState(""),[failed,setFailed]=useState(false),[retry,setRetry]=useState<Request|null>(null);
 const lock=useRef(false),alive=useRef(true),sequence=useRef(0),clockBase=useRef({server:Date.parse(initial.server_time),local:0});
 const clock=useCallback(()=>clockBase.current.server+(clockBase.current.local?performance.now()-clockBase.current.local:0),[]);
 const accept=useCallback((next:RangeState,sent:number)=>{clockBase.current={server:Date.parse(next.server_time)+(performance.now()-sent)/2,local:performance.now()};setData(next);},[]);
 ...
 try{const r=await createClient().rpc("range_action",{p_action:request.action,p_payload:request.payload});if(r.error)throw new Error("The shot reply was interrupted. Retry safely before firing again.");uncertain=false;setRetry(null);if(r.data?.error)throw new Error(r.data.error);if(alive.current){accept(r.data.state,sent);setNotice(r.data.message);}return true;
 ...
 return {data,working,notice,failed,retry,busy:working||!!retry,clock,refresh:()=>void refresh().catch(e=>{setFailed(true);setNotice(e.message);}),act:(action:string,payload:Record<string,unknown>)=>retry?Promise.resolve(false):run({action,payload:{...payload,request_id:crypto.randomUUID()}}),retryAction:()=>retry?run(retry):Promise.resolve(false)};
}
