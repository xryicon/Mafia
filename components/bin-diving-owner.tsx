...
export function BinDivingOwner(){
 ...
 {!data?<p>Loading city rules… <button onClick={h.refresh}>Refresh</button></p>:!data.can_manage?<p>Owner permission required.</p>:<><p>Control scavenging loot across every open district. Bins and successfully opened cars use these chances. Each car attempt consumes one lockpick.</p><p><Link href="/owner?section=economy">Edit scavenging walk time, search time, restock time, car success chance and Lockpicking XP in Economy settings →</Link></p><RulesForm key={data.rules.version} initial={data.rules} busy={h.busy} save={(r,reason)=>h.act("configure",{...r,reason})}/><button className="bin-outline" disabled={h.busy} onClick={h.refresh}>Refresh current rules</button><p><Link href="/owner?section=adjustments">Give a player loot through Asset grants →</Link></p></>}
 </section>;
}
