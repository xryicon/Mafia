...
function RulesForm({initial,busy,save}:{initial:BinRules;busy:boolean;save:(rules:BinRules,reason:string)=>Promise<boolean>}){
 ...
 return <form className="bin-rules-form" onSubmit={async e=>{e.preventDefault();if(await save(rules,reason))setReason("");}}>
 ...
 <label>Reason for this change<textarea required minLength={5} maxLength={500} value={reason} onChange={e=>setReason(e.target.value)} placeholder="Describe the balance change for your audit history."/></label>
 <button className="bin-gold" disabled={busy||total>100||rules.cash_max<rules.cash_min}>Save bin diving rules →</button>
 <p className="bin-muted">Changes affect future dives. Existing cooldowns and awarded loot stay as recorded. Pickaxe condition uses your existing mining setting. New open districts are included automatically.</p>
 </form>;
}
