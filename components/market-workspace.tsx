...
import {auctionTime,nextBid,type Auction,type MarketState,type MarketView} from "@/lib/market";
...
function Deal({offer,auction,game,busy,onClose,onBuy,onBid}:{offer:Listing|null;auction:Auction|null;game:MarketState["game"];busy:boolean;onClose:()=>void;onBuy:()=>void;onBid:(amount:number)=>void}){
 ...
 return <dialog ref={dialog} className="command-modal market-deal" aria-labelledby="market-deal-title" onCancel={e=>{if(busy)e.preventDefault();else onClose();}}>
  ...
  <div className="market-deal-commodity"><CommodityArtwork goodId={auction?.good_id??offer?.good_id??""} size={88}/><p>{auction?.quantity??offer?.quantity} {name}<small>{auction?.seller_name??offer?.seller_handle}</small></p></div>
...
function SellTicket({data,good,district,busy,onSelect,onAction}:{data:MarketState;good:string;district:string;busy:boolean;onSelect:(id:string)=>void;onAction:Action}){
 ...
 return <Panel title="Create a listing" className="market-ticket">
  ...
  <div className="market-stock-preview"><CommodityArtwork goodId={good} size={96}/><div><small>SELECTED COMMODITY</small><strong>{data.game.goods.find(g=>g.id===good)?.name??"Select goods"}</strong></div></div>
...
export function MarketWorkspace({initial,initialView,initialGood,initialDistrict}:{initial:MarketState;initialView:MarketView;initialGood?:string;initialDistrict?:string}){
 ...
 const [fixedPage,setFixedPage]=useState(0),[auctionPage,setAuctionPage]=useState(0),[inventoryPage,setInventoryPage]=useState(0);
 ...
 useEffect(()=>{setFixedPage(0);setAuctionPage(0);setInventoryPage(0);},[view,filter,district,search,sort]);
 ...
 const inventoryGoods=game.goods.filter(g=>filter==="all"||filter===g.id);
 const fp=Math.min(fixedPage,Math.max(0,Math.ceil(fixed.length/10)-1)),ap=Math.min(auctionPage,Math.max(0,Math.ceil(auctions.length/10)-1)),ip=Math.min(inventoryPage,Math.max(0,Math.ceil(inventoryGoods.length/10)-1));
 function offers(){return fixed.length?<div className={"market-book "+(view==="mine"?"own-offers":"")}>{fixed.slice(fp*10,fp*10+10).map(l=><article className="market-lot" key={l.id}>
  <div className="market-commodity"><CommodityArtwork goodId={l.good_id} size={88}/><div><strong>{name(l.good_id)}</strong><Link href={"/players/"+l.seller_id}>{l.seller_id===game.player.id?"Your listing":l.seller_handle}</Link></div></div>
 ...
 function auctionRows(){return auctions.length?<div className="market-auction-grid">{auctions.slice(ap*10,ap*10+10).map(a=>{
  ...
  return <article className={"market-auction "+(leading?"leading":"")} key={a.id} data-auction={a.id}><div className="market-auction-top"><span className="market-tag">{a.status==="open"?(leading?"YOU'RE LEADING":yours?"YOUR AUCTION":"LIVE AUCTION"):a.status.toUpperCase()}</span><time dateTime={a.ends_at}>{a.status==="open"?auctionTime(a.ends_at,now):new Date(a.completed_at??a.ends_at).toLocaleDateString("en-GB")}</time></div>
   <div className="market-commodity"><CommodityArtwork goodId={a.good_id} size={88}/><div><strong>{name(a.good_id)}</strong><small>{a.quantity.toLocaleString("en-US")} units · <Link href={"/players/"+a.seller_id}>{a.seller_name}</Link></small></div></div>
 ...
 return <div className="market-workspace">
  ...
  <div className="market-columns">
   <aside className="market-left"><Panel title="The trading desk"><nav className="market-sections" aria-label="Market sections">{([["floor","Trading floor","trade"],["auctions","Auctions","auction"],["mine","My listings","ledger"],["inventory","Inventory","inventory"],["orders","Buy orders","cash"],["history","Price history","ledger"]] as const).map(([id,label,icon])=><button key={id} aria-current={view===id?"page":undefined} onClick={()=>go(id)}><GameIcon name={icon} size={18}/>{label}<GameIcon name="arrow" size={14}/></button>)}</nav></Panel>
    <Panel title="Commodities" className="market-commodities"><button className={filter==="all"?"selected":""} onClick={()=>setFilter("all")}>All commodities<span>{game.goods.length}</span></button>{game.goods.map(g=><button key={g.id} className={filter===g.id?"selected":""} onClick={()=>{setFilter(g.id);setGood(g.id);}}><CommodityArtwork goodId={g.id} size={48}/>{g.name}<span>{game.inventory.find(i=>i.good_id===g.id)?.quantity??0}</span></button>)}</Panel>
   ...
   <section className="market-main" aria-label="Exchange listings">
    {view!=="history"&&<div className="command-panel market-search"><label><span className="sr-only">Search goods or seller</span><GameIcon name="search" size={16}/><input placeholder="Search commodities or a player…" value={search} onChange={e=>setSearch(e.target.value)}/></label><label><span className="sr-only">District market</span><select value={district} onChange={e=>setDistrict(e.target.value)}><option value="">All districts</option>{data.districts.map(d=><option key={d.id} value={d.id}>{d.name}</option>)}</select></label><label><span className="sr-only">Sort listings</span><select value={sort} onChange={e=>setSort(e.target.value)}><option value="latest">Latest / ending soon</option><option value="price">Lowest unit price</option></select></label></div>}
    {view==="history"&&<MarketPriceHistory key={good} good={good} goods={game.goods} onSelect={setGood}/>}
    {(view==="floor"||view==="mine")&&<Panel title={view==="mine"?"Your fixed-price listings":"Fixed-price offers"} className="market-floor"><p className="market-section-note">{fixed.length} offer{fixed.length===1?"":"s"} · whole lots · prices set by players</p>{offers()}<MarketPagination page={fp} total={fixed.length} onPage={setFixedPage} label="Fixed-price offers"/></Panel>}
    {(view==="auctions"||view==="mine"||view==="floor")&&<Panel title={view==="mine"?"Your auctions & bids":"The auction room"}><div className="market-auction-heading"><p>Goods with a deadline. Influence with a price.</p><span className="market-tag">{auctions.filter(a=>a.status==="open").length} OPEN</span></div>{auctionRows()}<MarketPagination page={ap} total={auctions.length} onPage={setAuctionPage} label="Auctions"/></Panel>}
    {view==="inventory"&&<Panel title="Your warehouse"><p className="market-section-note">Available stock · choose goods to create a listing</p><div className="market-inventory">{inventoryGoods.slice(ip*10,ip*10+10).map(g=><button key={g.id} className={good===g.id?"selected":""} onClick={()=>setGood(g.id)}><CommodityArtwork goodId={g.id} size={144}/><strong>{g.name}</strong><span>{game.inventory.find(i=>i.good_id===g.id)?.quantity??0} units</span><small>Select to sell or auction <GameIcon name="arrow" size={13}/></small></button>)}</div><MarketPagination page={ip} total={inventoryGoods.length} onPage={setInventoryPage} label="Inventory"/></Panel>}
    {view==="orders"&&<DistrictBuyOrders key={district} districtId={district} game={game} onChange={reload} filter={filter} search={search}/>}
