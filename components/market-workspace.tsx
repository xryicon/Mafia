 ...
 return <div className="market-workspace">
  ...
  <div className="market-columns">
   ...
   <section className="market-main" aria-label="Exchange listings">
    ...
    {view==="inventory"&&<Panel title="Your warehouse"><p className="market-section-note">Available stock · choose goods to create a listing</p><div className="market-inventory">{game.goods.filter(g=>filter==="all"||filter===g.id).map(g=><button key={g.id} className={good===g.id?"selected":""} onClick={()=>setGood(g.id)}><CommodityArtwork goodId={g.id} size={144}/><strong>{g.name}</strong><span>{game.inventory.find(i=>i.good_id===g.id)?.quantity??0} units</span><small>Select to sell or auction <GameIcon name="arrow" size={13}/></small></button>)}</div></Panel>}
