...
export function SiteChrome({children}:{children:React.ReactNode}){
 const path=usePathname(),game=["/dashboard","/players","/seasons","/owner","/staff","/support","/community","/account"].some(p=>path===p||path.startsWith(p+"/")),home=path==="/",auth=["/login","/signup","/forgot-password","/update-password"].includes(path);
