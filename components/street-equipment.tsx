...
export function StreetEquipment({pause,revision}:{pause:()=>void;revision:string}){
 ...
 <div className="fp-equipment-slots">{equipmentSlots.map(([id,label,icon])=>{const gear=data?.gear.find(g=>g.location==='equipped'&&g.equipment_slot===id),good=data?.goods.find(g=>g.id===gear?.good_id);return <button key={id} aria-label={label+(good?': '+good.name:data?', empty':', loading')} aria-pressed={selected===id} title={label+(good?': '+good.name:'')} onClick={()=>{pause();setSelected(selected===id?null:id);}}><span>{label.replace('Primary weapon','Primary').replace('Secondary weapon','Sidearm')}</span>{good?<CommodityArtwork goodId={good.id} size={44}/>:<GameIcon name={icon} size={25}/>}<small>{gear?gear.quantity>1?'×'+gear.quantity:gear.condition!==null?String(gear.condition):'Equipped':error?'?':data?'—':'…'}</small></button>;})}<Link href="/inventory" onClick={pause} aria-label="Open Inventory" title="Open Inventory"><GameIcon name="inventory" size={23}/><span>Inventory</span></Link></div>
 </aside>;
}
