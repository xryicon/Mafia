...
export function StreetEquipment({pause,revision,activeSlot=0,onSelect,onHeld}:{pause:()=>void;revision:string;activeSlot?:number;onSelect?:(slot:number)=>void;onHeld?:(name:string|null)=>void}){
 ...
 useEffect(()=>{if(!data)return;const slot=equipmentSlots[activeSlot]?.[0],g=data.gear.find(g=>g.location==='equipped'&&g.equipment_slot===slot),good=data.goods.find(x=>x.id===g?.good_id);onHeld?.(slot==='primary'||slot==='secondary'?good?.name??null:null);},[activeSlot,data,onHeld]);
 ...
 <div className="fp-equipment-slots">{equipmentSlots.map(([id,label,icon],index)=>{const gear=data?.gear.find(g=>g.location==='equipped'&&g.equipment_slot===id),good=data?.goods.find(g=>g.id===gear?.good_id);return <button key={id} data-active={activeSlot===index} aria-label={label+(good?': '+good.name:data?', empty':', loading')} aria-pressed={selected===id} title={label+(good?': '+good.name:'')} onClick={()=>{onSelect?.(index);pause();setSelected(selected===id?null:id);}}><span>{index+1} · {label.replace('Primary weapon','Primary').replace('Secondary weapon','Sidearm')}</span>{good?<CommodityArtwork goodId={good.id} size={44}/>:<GameIcon name={icon} size={25}/>}<small>{gear?gear.quantity>1?'×'+gear.quantity:gear.condition!==null?String(gear.condition):'Equipped':error?'?':data?'—':'…'}</small></button>;})}<Link href="/inventory" onClick={pause} aria-label="Open Inventory" title="Open Inventory"><GameIcon name="inventory" size={23}/><span>Inventory</span></Link></div>
 </aside>;
}
