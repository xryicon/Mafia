...
export function BankWorkspace({initial}:{initial:BankState}){
 ...
 <div className="bank-account-actions"><button className="bank-gold" disabled={h.busy||!depositsOpen||d.player.cash<1} onClick={()=>open("deposit")}><span>↓</span> Deposit funds</button><button className="bank-outline" disabled={h.busy||!d.playable||d.balance<1} onClick={()=>open("withdraw")}><span>↑</span> Withdraw funds</button></div>
