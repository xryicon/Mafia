...
function MarketPrices({game}:{game:DashboardData["game"]}){
 ...
 return <Panel title="Market Prices" className="command-market-prices" action={<More href="/market">View Market</More>}>
  ...
  <div className="command-prices">{rows.slice(current*5,current*5+5).map(({good,price,offers})=><Link href={"/market?good="+encodeURIComponent(good.id)} key={good.id}><CommodityArtwork goodId={good.id} size={46}/><span>{good.name}</span><strong>{money(price)}</strong><small>{offers} {offers===1?"offer":"offers"}</small></Link>)}</div>
...
export function CommandDashboard({initial}:{initial:DashboardData}){
 ...
 return <div className="command-dashboard">
  ...
  {notice&&<div className={"command-notice"+(failed?" error":"")} role={failed?"alert":"status"}>{powerText(notice)}<button aria-label="Dismiss dashboard message" onClick={()=>setNotice("")}>×</button></div>}
