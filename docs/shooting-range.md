...
The range uses the original illustrated target view, with Advanced as the only option for new practice. Existing sessions retain their stored rules and historical results remain intact. Accepted shots display precision and points; the round countdown bar and reload countdown follow session timing. Automatic M4 fire, sound, paper damage, accuracy, ammunition, weapon wear and XP use the existing server rules. The 3D view and saved view preference are no longer used.
